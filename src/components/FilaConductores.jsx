...
export default function FilaConductores({ data }) {
    ...
    const {error: dataError, loading, deleteData} = useData(`${import.meta.env.VITE_ENDPOINT_BASE}/CarneConductor`);
            ...
            {data.map(({ id_conductor, nombre_conductor, apellido_conductor, dni_conductor, codigo_carne, fecha_emision, fecha_caducidad, foto_conductor }) => (
                <tr key={id_conductor}>
                    ...
                    <td className="px-6 py-4 text-center whitespace-no-wrap">
                        ...
                        <Link onClick={(e)=>handleSubmitDelete(e, id_conductor, nombre_conductor)} className="bg-red-500 text-white hover:bg-red-600 px-2 py-1 rounded-full">
